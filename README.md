...
- `public/admin.html` + `public/js/admin.js` — admin dashboard:
  - **"Security Data"** (one sidebar item, two tabs):
    ...
    - **Security Information** tab — reads live from MongoDB with a **search box** (name / roll no / mobile / designation), a **rows-per-page dropdown** (10/20/50/100), a **serial number column**, **Previous/Next** pagination, a **profile photo column** (the guard's uploaded photo, or their first initial if they haven't uploaded one — click it to change their photo without leaving this tab), a **Status** column (Active/Blocked), per-row **✏️ Edit / 🚫 Block / 🗑 Delete** actions on the right, a **Download Excel** button, and a **Download PDF** button that exports the same list as a PDF table with each person's actual profile photo embedded in a Photo column (falling back to an initial-letter avatar for anyone without one). Edit opens a small modal; Block and Delete ask for confirmation first.
